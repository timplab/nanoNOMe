#!/bin/bash
root=/dilithium/Data/NGS/Aligned/180215_nomeseq/yeast
beddir=$root/bed
tssdir=$root/tss
[ -e $tssdir ]||mkdir $tssdir
tss="/home/isac/Dropbox/Data/nome-seq/db/yeast/SacCer3_TSS.bed"
...
for bedgraph in `find $beddir -name "*bedGraph.gz"`;do
  ...
  if [ "$1" == "getdistance" ];then
    gunzip -c $bedgraph |\
      ...
      awk '{ OFS="t" }{ if(($NF<5000)&&($NF>-5000)) print }' >  $tssdir/$base.gpc.distance2tss.bed
  fi
