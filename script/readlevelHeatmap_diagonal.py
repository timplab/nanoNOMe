...
class HeatmapRegion :
    ...
    def plot(self,thr,window) :
        ...
        # mask top 0.2%
        qthr = np.quantile(df_meth['z'],.998)
        df_meth['z'][df_meth['z']>=qthr] = qthr
        df_meth['z'] = df_meth['z']/qthr
        qthr = np.quantile(df_unmeth['z'],.998)
        df_unmeth['z'][df_unmeth['z']>=qthr] = qthr
        df_unmeth['z'] = df_unmeth['z']/qthr
        ...
        g = (ggplot(df) +
                facet_grid(['lab','.'])+
                geom_tile(aes(x='x',y='y',fill='z'),size=0.1) +
