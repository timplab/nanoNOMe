...
def parseArgs() :
    ...
    # parser
    parser = argparse.ArgumentParser(description='parse MNase-seq or DNAseq-seq data')
    ...
    parser_region_meth.add_argument('-i','--input',type=argparse.FileType('r'),
            ...
            help="input DNAse-seq/MNase-seq signal-to-region distance file (done by bedtools closest) (default stdin)")
    parser_region_meth.add_argument('-w','--window',type=int,
            required=False,default=2000,help="window")
    parser_region_meth.set_defaults(func=SignalByDistance_main)
    # parse args
    args = parser.parse_args()
    args.srcdir=srcdir
    return args

def SignalByDistance_main(args) :
    ...
    if args.verbose : print("avergaing with respect to distance",file=sys.stderr)
