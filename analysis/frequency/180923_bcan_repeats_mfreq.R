...
outdir=file.path(srcdir,"../../annotations/")
outpath=file.path(outdir,"bcan_10a_vs_231_repeats.bed")
...
bcanrep = file.path(root,"annotations/breastcancer/repeats_bcan_proximity.bed")

lab = "mfreq"
dbpath = file.path(root,"annotations/breastcancer/repeats_bcan_proximity.bed")

dat.list = mclapply(mc.cores=cores,seq(dim(pd)[1]),function(i){
    tabix_mfreq(pd$filepath[i],dbpath)
})
db.gr = load_db(dbpath,extracols=c("reptype",
                                   "genechrom","genestart","geneend","geneid",
                                   "genescore","genestrand","genename","fxn","dist"))
