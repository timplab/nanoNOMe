...
outdir=/dilithium/Data/NGS/Aligned/181003_methylationStandards/read1
...
if [ "$1" == "trim" ];then
  ...
  com="trim_galore -v > $log &&\
    trim_galore --hardtrim5 50 \
    $fq1 -o $trimdir &>> $log"
  parallel "$com" ::: $na $ecoli
  fq1=$trimdir/{}_*bp.fq.gz
  com="trim_galore --fastqc \
    $fq1 -o $trimdir &>> $log"
  parallel "$com" ::: $na $ecoli
fi
...
if [ "$1" == "align" ];then
  ...
  for samp in $na $ecoli;do
    echo $samp
    fq1=$(find $trimdir -name "${samp}_*trimmed.fq.gz")
...
if [ "$1" == "extract" ];then
  ...
  for bam in $(find $bamdir -name "*nodupqsort.bam");do
    ...
    args="--ignore 2 --ignore_3prime 1"
    com="bismark_methylation_extractor -s --multicore 8 --gzip \
...
if [ "$1" == "report" ];then
  ...
  for samp in $na $ecoli;do
    ...
    bismark2report --dir $reportdir \
      --alignment_report ${pre}_SE_report.txt \
