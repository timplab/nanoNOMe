...
nums.tb <- diffregs %>%
  filter(sig == "sig") %>%
  group_by(mod,direction) %>%
  summarize(n())
plt <- diffregs %>%
  group_by(mod,lowpval) %>%
  sample_n(5e3)

plotpath <- file.path(plotdir,"200802_gm12878_diffregs_volcano.pdf")
pdf(plotpath,width = 4, height = 4, useDingbats = F)
ggplot(plt,aes(x = meandiff, y = -log10(p.value), color = sig)) +
...
outpath <- file.path(plotdir,"200801_haplotype_DMR_DAR_gene_promoters.tsv")
...
outpath <- file.path(plotdir,"200801_diffmeth_gene_promoters.tsv")
...
plotpath <- file.path(plotdir,"200801_gm12878_haplotype_promtoer_chroms.pdf")
...
# plot metaplot
plotpath <- file.path(plotdir,"200801_gm12878_haplotype_promoter_upset_include_none.pdf")
...
pal <- wes_palette("Zissou1")
hap_pal <- c(pal[1],pal[4])
plotpath <- file.path(plotdir,"200801_prom_concordant_region.pdf")
...
pal_xci <- c(pal[9],pal[4],pal[8])
# plot metaplot
plotpath <- file.path(plotdir,"200801_gm12878_haplotype_promoter_validation_metaplot.pdf")
...
regmeth.tb %>%
  group_by(genetype,mod) %>%
  summarize(n())
plotpath <- file.path(plotdir,"200801_gm12878_haplotype_promoter_validation.pdf")
...
ggplot(regmeth.tb,aes( x = methdiff, fill = genetype)) +
  ...
  labs( x = "Methylation Difference ( Pat - Mat)", y = "Density") +
...
plotpath <- file.path(plotdir,"200801_gm12878_haplotype_promoter_validation_pairwise.pdf")
...
# Let's save the data as a table to provide as a supp table
out.tb <- plt %>%
  dplyr::select(-direction,-frac,-expected,-enrichment,-nadjust) %>%
  dplyr::rename(Type = mod, Context = type, Higher = Direction, Number = n) %>%
  mutate(Type = case_when(
      Type == "konc" ~ "Concordant",
      Type == "cpg" ~ "DMR",
      Type == "gpc" ~ "DAR"),
    Enrichment = round(Enrichment,3),
    ) %>%
  pivot_wider(names_from = Higher, values_from = c(Enrichment,Number), names_sep = " in ")
outpath <- file.path(plotdir,"200802_gm12878_haplotype_auto_genome_context_bar_data.tsv")
write_tsv(out.tb,outpath)
plotpath <- file.path(plotdir,"200802_gm12878_haplotype_auto_genome_context_bar.pdf")
pdf(plotpath, height = 6, width = 4,useDingbats = F)
ggplot(plt,aes(x = type, fill = Direction,y = Enrichment)) +
