...
onei <- which(cpg.pd$allele == "maternal")
twoi <- which(cpg.pd$allele == "paternal")
...
gpc.pd <- pData(gpc.keep)
onei <- which(gpc.pd$allele == "maternal")
twoi <- which(gpc.pd$allele == "paternal")
dars <- compareRegions(gpc.keep,onei,twoi, regions = peaks.cand)

# output the results
# first as rds
diffregs.list <- list(dmrs = dmrs,dars = dars)
outpath <- file.path(fdir,"200225_gm12878_haplotype_diffregs.rds")
if (F){
...
plotpath <- file.path(plotdir,"200225_gm12878_diffregs_volcano.pdf")
...
if (F) {
  # save them as tsv
  dmrs.out <- dmrs.sig %>%
    mutate(higher = ifelse(direction == "up","Maternal","Paternal")) %>%
    dplyr::select(
      Chromosome = chr,
      Start = start,
      End = end,
  ...
  dmrs.tsv <- file.path(plotdir,"200425_gm12878_hapltype_dmrs_sig.tsv")
  write_tsv(dmrs.out,dmrs.tsv)
  dars.tsv <- file.path(plotdir,"200425_gm12878_hapltype_dars_sig.tsv")
  write_tsv(dars.out,dars.tsv)
}

# make granges of significant
...
plotpath <- file.path(plotdir,"200227_gm12878_haplotype_dmr_dar_overlap_upset.pdf")
...
plotpath <- file.path(plotdir,"200309_dmr_region_eg.pdf")
...
pal <- wes_palette("Zissou1")
hap_pal <- c(pal[1],pal[4])
plotpath <- file.path(plotdir,"200309_dar_region_eg.pdf")
pdf(plotpath,height = 3, width = 4, useDingbats = F)
ggplot(meth.tb,aes( x = start, y = freq, color = allele)) +
  facet_wrap(~what,nrow = 2,scales = "free") +
  geom_line() +
...
```{r diff_auto_vs_x, eval = T, echo = F, fig.height=6, fig.width=8, message=F, warning = F,results = 'hide', fig.show = 'show'}
table(dmrs.sig$chr == "chrX")
table(dars.sig$chr == "chrX")
...
dars.sig %>%
  ...
  summarize( n = n())
dmrs.sig %>%
  dplyr::select(direction,average_one,average_two)
```
Interestingly, there are more hypermethylated regions in active chrom (down, b/c comparison is pat relative to mat), let's look at promoter regions
...
## which of them actually have data?
#tssregs.gr <- resize(tss.gr,width = 1000, fix = "center")
#cpgovl <- findOverlaps(cpg.keep,tssregs.gr)
#gpcovl <- findOverlaps(gpc.keep,tssregs.gr)
## at least 10 sites?
#thr <- 5
...
#  filter( n >= thr) %>%
#  .$subjectHits
#tss$cpg <- tss$gpc <- "nocov"
#tss$cpg[cpgidx] <- "covered"
#tss$gpc[gpcidx] <- "covered"
#covered_num <- tss %>%
#  mutate(covered = ifelse(cpg == "covered" & gpc == "covered","covered","not")) %>%
#  group_by(covered,genetype) %>%
#  summarize(n = n()) %>%
#  spread(covered,n) %>%
...
## annotate partially active genes?
#tss.all <- tss
#prommeth.gather <- prommeth.tb %>%
#  dplyr::select(mod,tssidx,pat_freq,mat_freq) %>%
#  gather(hap,freq,pat_freq,mat_freq) %>%
#  mutate(hap = ifelse(hap == "pat_freq","paternal","maternal"),
#    active = ifelse( (mod == "cpg" & freq < 0.25) | 
#      ( mod == "gpc" & freq > 0.5),"active","inactive"))
#active.tss <- prommeth.gather %>%
...
#  distinct(tssidx)
#tss$active <- "inactive"
#tss$active[active.tss$tssidx] <- "active"
#tss.all <- tss
#
#tss <- tss %>% filter(active == "active")
#tss.gr <- GRanges(tss)
#
#pdf(file.path(plotdir,"200322_gm_tss_meth_distros.pdf"))
#ggplot(prommeth.gather,aes(x = freq)) +
#  facet_wrap(~mod) +
#  geom_density()
#dev.off()
#
...
outpath <- file.path(plotdir,"200226_diffmeth_gene_promoters.tsv")
write_tsv(tssmeth.out,outpath)
## how many distinct genes have DM or DA
#tssmeth.gene <- tssmeth.tb %>%
#  distinct(Gene_Symbol,`Higher CpG`, `Higher GpC`,.keep_all = T)  
#gene.num <- tssmeth.gene %>%
#  filter(`Higher CpG` != "None" | `Higher GpC` != "None") %>%
#  distinct(Gene_Symbol,.keep_all = T) %>%
#  group_by(genetype) %>%
#  summarize( n= n())
#gene.tot <- sum(gene.num$n)
...
plotpath <- file.path(plotdir,"200318_gm12878_haplotype_promtoer_chroms.pdf")
...
### ASE
```{r ase, eval = T, echo = F, fig.height=6, fig.width=8, message=F, warning = F,results = 'hide', fig.show = 'show'}
tssall.gr <- GRanges(tss.all)
dmrs.dist <- as.data.frame(distanceToNearest(tssall.gr,dmrsig.gr))
dars.dist <- as.data.frame(distanceToNearest(tssall.gr,darsig.gr))

tss.dmrdist <- tss.all[dmrs.dist$queryHits,] %>%
  mutate( diffidx = dmrs.dist$subjectHits,
    dist = dmrs.dist$distance,
    direction = dmrs.sig$direction[diffidx],
    meandiff = dmrs.sig$meandiff[diffidx]
  )
tss.dardist <- tss.all[dars.dist$queryHits,] %>%
  mutate( diffidx = dars.dist$subjectHits,
    dist = dars.dist$distance,
    direction = dars.sig$direction[diffidx],
    meandiff = dars.sig$meandiff[diffidx]
  )
...
dist.both <- tss.dist %>%
  dplyr::select(-bin,-diffidx,-direction,-meandiff) %>%
  spread(mod,dist) %>%
  rowwise() %>%
  mutate(
    mod = "both",
...
dist.all <- bind_rows(tss.dist,dist.both)

#  close ones wrt ase
tss.all %>%
  mutate(chrx = ifelse(chrom == "chrX",T,F)) %>%
  group_by(chrx,ase) %>%
  summarize(n())
both.ase <- tss.dist %>%
  filter(dist <= 500) %>%
  dplyr::select(chrom,hgnc,tssidx,ase,mod,meandiff) %>%
  spread(mod,meandiff) %>%
  filter(!is.na(dar) , !is.na(dmr)) %>%
  filter(sign(dar) != sign(dmr)) %>%
  mutate(direction = ifelse(dar > 0,"Paternal","Maternal"))
both.ase %>%
  filter(tssidx %in% tss$tssidx) %>%
  group_by(ase,direction) %>%
  summarize(n =n())
both.ase %>%
#  filter(chrom != "chrX") %>%
  group_by(ase,direction) %>%
...
close.genes <- dist.all %>%
  filter(dist <= 500) %>%
  dplyr::select(mod,ensid,hgnc,tssidx,ase,genetype,dist) %>%
  spread(mod,dist)  %>%
  filter(!is.na(both))
asedist.frac <- dist.all %>%
  mutate(genetype = ifelse(ase %in% c("Paternal","Maternal"),"ASE",ase)) %>%
  filter(!is.na(ase)) %>%
...
pdf(plotpath,width = 4, height = 4,useDingbats = F)
ggplot(asedist.frac,aes( x = bin, y = cumfrac, color = genetype)) +
...
plotpath <- file.path(plotdir,"200318_gm12878_haplotype_chrx_genome_context_bar.pdf")
...
# separate by context
plotpath <- file.path(plotdir,"200318_gm12878_haplotype_chrx_genome_context_bar_separate.pdf")
...
# 
# log ratio
plotpath <- file.path(plotdir,"200318_gm12878_haplotype_chrx_genome_context_logratio.pdf")
pdf(plotpath, height = 3, width = 4,useDingbats = F)
...
Allele-specific long-range interactions?

```{r hic, eval = T, echo = F, fig.height=6, fig.width=8, message=F, warning = F,results = 'hide', fig.show = 'show'}
hicpath <- "/dilithium/Data/NGS/projects/gm12878/hic/encode/ENCFF773BSY.bedpe.gz"
hic <- read_tsv(hicpath)
hic.one <- hic %>% dplyr::select(chrom = `#chr1`, start = centroid1, end = centroid1) %>%
  mutate(hicidx = seq(nrow(hic)))
hic.two <- hic %>% dplyr::select(chrom = chr2,start = centroid2, end = centroid2) %>%
  mutate(hicidx = seq(nrow(hic)))
hic.gr <- bind_rows(list(one = hic.one, two = hic.two),.id = "side") %>%
  GRanges()
hicregs.gr <- resize(hic.gr,width = 5e3,fix = "center")

# enrichment?
cpgsize <- sum(cpg.clusters$end - cpg.clusters$start)
gpcsize <- sum(gpc.clusters$end - gpc.clusters$start)
hicsize <- sum(width(hicregs.gr))

dmrhic <- sum(width(intersect(dmrsig.gr,hicregs.gr)))
darhic <- sum(width(intersect(darsig.gr,hicregs.gr)))

dmrhic/sum(width(dmrsig.gr)) /(hicsize/cpgsize)
darhic/sum(width(darsig.gr)) /(hicsize/gpcsize)

# overlap 

dmrovl <- findOverlaps(hicregs.gr,dmrsig.gr)
darovl <- findOverlaps(hicregs.gr,darsig.gr)

dmr.hic <- tibble(
  hicidx = hicregs.gr$hicidx[queryHits(dmrovl)],
  side = hicregs.gr$side[queryHits(dmrovl)],
  meandiff = dmrs.sig$meandiff[subjectHits(dmrovl)]) %>%
  group_by(hicidx,side) %>%
  filter(abs(meandiff) == max(abs(meandiff))) %>%
  ungroup() 

dar.hic <- tibble(
  hicidx = hicregs.gr$hicidx[queryHits(darovl)],
  side = hicregs.gr$side[queryHits(darovl)],
  meandiff = dars.sig$meandiff[subjectHits(darovl)]) %>%
  group_by(hicidx,side) %>%
  filter(abs(meandiff) == max(abs(meandiff))) %>%
  ungroup() 

dmrhic.spread <- dmr.hic %>%
  spread(side,meandiff) %>%
  na.omit()
darhic.spread <- dar.hic %>%
  spread(side,meandiff) %>%
  na.omit()

# how do they look in general (w/o allele-specific)
cpg.hicregs <- cpgnonhap.keep[overlapsAny(cpgnonhap.keep,hicregs.gr),]
gpc.hicregs <- gpcnonhap.keep[overlapsAny(gpcnonhap.keep,hicregs.gr),]

meth <- bind_rows(list(cpg = tibble(freq = getMeth(cpg.hicregs)[,1]), 
  gpc = tibble(freq = getMeth(gpc.hicregs))[,1]),.id = "mod")

```

```{r hic_plots, eval = T, echo = F, fig.height=6, fig.width=8, message=F, warning = F,results = 'hide', fig.show = 'show'}
plotpath <- file.path(plotdir,"200310_hap_hic.pdf")
pdf(plotpath)
ggplot(meth,aes( x = freq, fill = mod)) +
  geom_density(alpha = 0.5)
dev.off()
```

```{r xist, eval = T, echo = F, fig.height=6, fig.width=8, message=F, warning = F,results = 'hide', fig.show = 'show'}
idx <- match("XIST",tss$hgnc)
tss.xist <- tss[idx,]
gr <- GRanges(tss.xist)
dmrs.sig[nearest(gr,dmrsig.gr ),]$direction
distanceToNearest(gr,dmrsig.gr)
dars.sig[nearest(gr,darsig.gr ),]$direction
distanceToNearest(gr,darsig.gr)
```

```{r ace2, eval = T, echo = F, fig.height=6, fig.width=8, message=F, warning = F,results = 'hide', fig.show = 'show'}
# for ncov-19, check ace2 
idx <- match("ACE2",genes$hgnc)
tss.sel <- tss[idx,]
gene.sel <- genes[idx,]
gr <- GRanges(tss.sel)
dmrs.sig[nearest(gr,dmrsig.gr ),]$direction
...
plotpath <- file.path(plotdir,"ace2_hap_plot.pdf")
pdf(plotpath)
# the entire gene
gr.reg <- resize(GRanges(gene.sel),width = 10000 + gene.sel$end - gene.sel$start + 1,fix = "center")
cpg.reg <- cpg.keep[overlapsAny(cpg.keep,gr.reg),]
gpc.reg <- gpc.keep[overlapsAny(gpc.keep,gr.reg),]

cgmeth <- as_tibble(getMeth(cpg.reg)) %>%
  bind_cols(as_tibble(granges(cpg.reg)))
names(cgmeth)[1:2] <- pData(cpg.reg)$allele
gcmeth <- as_tibble(getMeth(gpc.reg)) %>%
  bind_cols(as_tibble(granges(gpc.reg)))
names(gcmeth)[1:2] <- pData(gpc.reg)$allele

meth.tb <- bind_rows(list(cpg = cgmeth,gpc = gcmeth),.id = "mod")
meth.gather <- meth.tb %>%
  gather(allele,freq,paternal,maternal)

ggplot(meth.gather,aes( x = start, y = freq, color = allele)) +
  facet_wrap(~mod, nrow = 2) +
  geom_line() +
  geom_rug(sides = "b",color = 'black', alpha = 0.1) +
  geom_vline(xintercept = tss.sel$start,linetype = "dashed") +
  annotate("rect", xmin = gene.sel$start, xmax = gene.sel$end, ymin = -Inf, ymax = Inf,  fill = "orange", alpha = 0.3)  +
  lims(y = c(0,1))
# just around tss
gr.reg <- resize(gr,width = 5000,fix = "center")
cpg.reg <- cpg.keep[overlapsAny(cpg.keep,gr.reg),]
gpc.reg <- gpc.keep[overlapsAny(gpc.keep,gr.reg),]
...
colMeans(getCoverage(gpc.reg,type = "Cov"))
